...
begin
    function plot_yearly()
        ...
        plot(
            ...
            xticks = xticks,
            xrotation = -45,
            xformatter = x -> Dates.format(x, "yyyy"),
        )
    end
    plot_yearly()
end
...
begin
    function plot_yearly_bar()
        ...
        bar(
            ts_yearly_pos.Year .+ year_offset,
            ts_yearly_pos.STOCK1_sum,
            color = :lightgreen,
        ...
        bar!(
            ts_yearly_neg.Year .+ year_offset,
            ts_yearly_neg.STOCK1_sum,
            color = :red,
...
begin
    function plot_monthly_bar()
        ...
        bar(
            ts_monthly_pos.Year_Month,
            ts_monthly_pos.STOCK1_sum,
            color = :lightgreen,
            label = "positive ret",
            legend = :topright,
            bar_width = bar_width,
        ...
        bar!(
            ts_monthly_neg.Year_Month,
            ts_monthly_neg.STOCK1_sum,
            color = :red,
            label = "negative ret",
            bar_width = bar_width,
        )
...
(
    return_year_month.Year,
    return_year_month.Month,
    Matrix(return_year_month_unstacked[!, Not(:Year)]),
)
...
begin
    function plot_heatmap()
        plt = heatmap(
            unique(return_year_month.Month),
            unique(return_year_month.Year),
            Matrix(return_year_month_unstacked[!, Not(:Year)]) .* 100.0,
            c = :redgreensplit,
            clims = (-ret_monthly_max, ret_monthly_max),
            xticks = 1:12,
        )  # :redgreensplit ou :RdYlGn_5
        for year in unique(return_year_month.Year)
            for month in unique(return_year_month.Month)
                val = return_year_month[
                    return_year_month.Year.==year.&&return_year_month.Month.==month,
                    :STOCK1_sum,
                ][1]
